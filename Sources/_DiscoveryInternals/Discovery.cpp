...
static SWTMachHeaderList getMachHeaders(void) {
  ...
  /// discovery library is not tasked with the same performance constraints as
  ...
  dispatch_once_f(&once, nullptr, [] (void *) {
    ...
    objc_addLoadImageFunc([] (const mach_header *mh) {
      ...
      // the discovery library when it calls enumerateTypeMetadataSections().
...
#else
#warning Platform-specific implementation missing: Runtime discovery unavailable
template <typename SectionEnumerator>
static void enumerateTypeMetadataSections(const SectionEnumerator& body) {}
#endif
